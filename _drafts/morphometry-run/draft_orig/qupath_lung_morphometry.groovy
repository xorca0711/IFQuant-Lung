...
if (CMP_AGER && (CHANNELS as List).contains(AGER_CH))
  logMsg("*** WARNING: IFQ_MORPH_CHANNELS includes the AGER channel " + AGER_CH + " while " +
         "IFQ_MORPH_COMPARE_AGER=true. The architecture measure is then NOT independent of the " +
         "denominator it is supposed to check, and any agreement is partly circular. ***")
int[] EPS
...
slides.each { slideFile ->
  ...
  if (CMP_AGER || PARTITION) {
    ...
    byte[] agerPos = new byte[cw * ch]
    for (int i = 0; i < af.length; i++)
      if (((roiC[i] & 0xFF) > 127) && af[i] >= AGER_THR) agerPos[i] = (byte) 255
    float[] dens = Morph.localFraction(agerPos, roiC, cw, ch, DAMAGE_SIGMA / pxCoarse)
  ...
  for (int by = 0; by < fh && !capped; by += coreF) {
    for (int bx = 0; bx < fw; bx += coreF) {
      ...
      // quick reject using the coarse ROI
      long inRoi = 0L
      for (int yy = by / K; yy < Math.min(ch, (by + chgt + K - 1) / K); yy++)
        for (int xx = bx / K; xx < Math.min(cw, (bx + cwid + K - 1) / K); xx++)
  ...
  (1..nRegions).each { int r ->
    ...
    double roiPxR = 0.0d
    for (int i = 0; i < labC.length; i++) if ((labC[i] & 0xFF) == r) roiPxR += 1.0d
    row["region_area_um2"] = roiPxR * coarseArea
    ...
    if (CONS_CUTOFF > 0) {
      double consPx = 0.0d
      for (int i = 0; i < labC.length; i++)
        if ((labC[i] & 0xFF) == r && (consC[i] & 0xFF) > 127) consPx += 1.0d
      row["morph_consolidated_positive_area_um2"] = consPx * coarseArea
    }
    if (stain2C != null) {
      // Masson / PSR collagen (or H&E eosin) area. Lands in the same
      // _positive_area_um2 family, so aggregate_to_mouse.py gives the pooled
      // area AND morph_bfstain2_positive_area_fraction for free.
      double s2px = 0.0d
      for (int i = 0; i < labC.length; i++)
        if ((labC[i] & 0xFF) == r && (stain2C[i] & 0xFF) > 127) s2px += 1.0d
      row["morph_bfstain2_positive_area_um2"] = s2px * coarseArea
    }
    if (CMP_AGER) {
    ...
    row["class_morph_perimeter_um_count"]      = Morph.croftonPerimeterUm(perAcc[r], pxFine)
    row["class_morph_chordlen_um_count"]       = chordAcc[r][0]
    row["class_morph_chordn_count"]            = chordAcc[r][1]
    row["class_morph_testline_um_count"]       = chordAcc[r][2]
    row["class_morph_transition_count"]        = chordAcc[r][3]
    row["class_morph_chordtrunc_count"]        = chordAcc[r][4]
    row["class_morph_chordtrunclen_um_count"]  = chordAcc[r][5]
    row["class_morph_chordlenh_um_count"]      = chordH[r][0]
    row["class_morph_chordnh_count"]           = chordH[r][1]
    row["class_morph_chordlenv_um_count"]      = chordV[r][0]
    row["class_morph_chordnv_count"]           = chordV[r][1]
    row["class_morph_edmhalf_um_count"]        = edmAcc[r][0]
