...
namespace IFQuantLauncher
{
    ...
    internal sealed partial class MainForm : Form
    {
        ...
        private void StartFijiRun(bool previewOnly)
        {
            ...
            // very same code path.
            EnvironmentApply.Apply(psi, config.Environment);
        ...
        private RunConfiguration ReadAndValidateConfiguration(bool previewOnly)
        {
            ...
            else
            {
                ...
                env = RunEnvironment.BuildStage2(
                    request, ResolveSelectedPanel(), engineThresholdMarkers,
            ...
            // H1/H3/H4, checked against the environment that will actually be
            // handed to the process rather than against the UI that produced it.
            //
            // The Advanced key set is passed for one rule: on route 4,
            // IFQ_MIN_INCLUDED_NUCLEI is legal in the environment if and only if
            // the operator typed it, because v1.7.2's Advanced box was the only
            // way to set the nuclei floor. The launcher writing it by itself is
            // still refused.
            PreStartAssertions.AssertStage2Environment(
                env, outputDirectory, route,
        ...
        /// Kept as a name so the route-2 stage runner reads the same as v1.7.2;
        /// the behaviour now lives in EnvironmentApply, which the legacy
        /// equivalence harness executes directly.
        private static void ClearIfqEnvironment(StringDictionary environment)
        {
            EnvironmentApply.ClearIfq(environment);
        }
