...
namespace IFQuantLauncher
{
    internal sealed partial class MainForm
    {
        ...
        private int UiSmokeBody()
        {
            ...
            if (!routeUnavailableLabel.Visible ||
                routeUnavailableLabel.Text.Length == 0) return 67;
        ...
        private PanelDef ResolveSelectedPanel()
        {
            ...
            PanelDef panel;
            return EnginePanels.TryGetValue(key, out panel) ? panel : null;
        }
        ...
        private void RebuildThresholdGridCore()
        {
            ...
            if (panel == null)
            {
                ...
                thresholdNoteLabel.Text = auto
                    ? "AUTO resolves a panel per image, so no channel threshold can be frozen " +
                      ...
                      "thresholds."
                    : "Select a built-in staining panel to see its analysis channels.";
                return;
            }

            thresholdNoteLabel.ForeColor = Color.FromArgb(75, 75, 75);
            thresholdNoteLabel.Text =
                "Marker thresholds for panel " + panel.Key + ". An empty box is NOT neutral: " +
            ...
            foreach (ChannelDef channel in panel.AnalysisChannels)
            {
                ...
                bool engineReadsIt = engineThresholdMarkers.Contains(channel.Token);
                ...
                string previous;
                if (retained.TryGetValue(channel.Token, out previous)) box.Text = previous;
        ...
        private void UpdateThresholdStatusLabels(PanelDef panel)
        {
            ...
            foreach (ChannelDef channel in panel.AnalysisChannels)
            {
                ...
                if (!engineThresholdMarkers.Contains(channel.Token))
                {
                    SetStatus(status,
        ...
        private GateResult EvaluateGate(RunRequest request)
        {
            ...
            // AUTO cannot freeze a threshold in advance, because the panel is
            // decided per image. That is a real property of the run, so it is
            // reported the same way a blank box is.
            if (request.PanelWasAuto && request.Route != ImageRoute.LegacyFiji172)
            {
                gate.Exploratory = true;
                gate.ThresholdPolicy.Add(
                    "ALL_CHANNELS=adaptive_otsu_exploratory(panel resolved per image by AUTO)");
                if (request.Tier == RunTier.Confirmatory)
                    gate.Findings.Add(new GateFinding(
                        Severity.Block, "H2_AUTO_IN_CONFIRMATORY",
                        "Confirmatory tier needs every analysis channel frozen, and AUTO decides " +
                        "the panel per image, so there is nothing to freeze in advance. Select " +
                        "the panel explicitly."));
                else
                {
                    gate.Findings.Add(new GateFinding(
                        Severity.Confirm, "H2_AUTO_ADAPTIVE",
                        "Panel selection is AUTO, so no channel threshold can be frozen before " +
                        "the run. Every channel will use per-region adaptive Otsu.",
                        FailClosedGate.ExploratoryStamp));
                    if (!gate.Blocked) gate.RequirePhrase(FailClosedGate.ExploratoryPhrase);
                }
                if (gate.Blocked) gate.RequiredPhrases.Clear();
            }
            return gate;
        }
            ...
            ThreadPool.QueueUserWorkItem(delegate
            {
                ...
                try
                {
                    ...
                    lastExit = RunStage(
                        ...
                        config.RuntimeDirectory, config.Stage1Environment);
                    if (lastExit != 0)
                    ...
                    if (failure == null)
                    {
                        ...
                        lastExit = RunStage(
                            ...
                            config.RuntimeDirectory, config.Environment);
                        string runManifest =
                    ...
                    if (failure == null && config.PythonExecutable != null)
                    {
                        ...
                        lastExit = RunStage(
                            ...
                            config.RuntimeDirectory, new Dictionary<string, string>());
